...
% Field label for multifield output names
field_label = getenv('FIELD_LABEL');
if isempty(field_label)
    field_label = 'unknown_field';
end
field_label = regexprep(field_label, '[^A-Za-z0-9_]+', '_');

out_prefix = fullfile(out_dir, ['noesypr1d_like_' field_label '_' matrix_stem '_' lb_tag]);
...
%   BF1_MHZ
%
% Bruker TD is real points; Spinach FID uses complex points = TD/2.
...
if isempty(SFO1_env) || isempty(O1_env) || isempty(SW_env) || isempty(TD_env)
...
if isempty(BF1_env)
    ...
    BF1_MHz = str2double(BF1_env);
end

if any(isnan([SFO1_MHz O1_Hz SW_h_Hz TD_real BF1_MHz]))
    error('One or more Bruker environment variables could not be converted to numbers.');
end
...
fprintf('Field label: %s\n', field_label);
fprintf('BF1 MHz: %.12f\n', BF1_MHz);
fprintf('SFO1 MHz: %.12f\n', SFO1_MHz);
fprintf('O1 Hz: %.6f\n', O1_Hz);
fprintf('SW_h Hz: %.6f\n', SW_h_Hz);
fprintf('TD real points: %d\n', TD_real);
fprintf('Spinach complex points: %d\n', n_complex);
...
% Spin system
% ------------------------------------------------------------
% Convert proton spectrometer frequency to magnetic field.
% 1H gamma/2pi ≈ 42.57747892 MHz/T.
